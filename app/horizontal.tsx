...
export default function HorizontalScreen() {
  ...
  return (
    <SafeAreaView style={listStyles.container}>
      ...
      <Text style={listStyles.title}>Horizontal Gallery</Text>

      <ScrollView horizontal showsHorizontalScrollIndicator={false}>
        {items.map((item) => (
          <View key={item.id} style={listStyles.cardHorizontal}>
            <Image
              source={{ uri: item.image }}
              style={listStyles.imageHorizontal}
            />
            <Text style={listStyles.cardText}>{item.title}</Text>
          </View>
        ))}
      </ScrollView>
    </SafeAreaView>
